    ...
    final consumersStream = FirebaseFirestore.instance
        ...
        .map((snap) {
          ...
          return snap.docs
              ...
              .whereType<RegisteredAccount>()
              .toList();
        });

    final upcoming = donor.scheduledDonations
        .where(
          (d) =>
              d.status == DonationScheduleStatus.scheduled &&
              !blocked.contains(d.consumerUid),
        )
        .toList();
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            t.dcGreeting(user.name.split(' ').first),
                            style: TextStyle(
                              color: isDark
                                  ? Colors.white
                    ...
                    InkWell(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            t.dcDefaultPickupTime(
                              donor.preferredPickupTime.format(context),
                            ),
                            style: TextStyle(
                              fontSize: 12.5,
                              fontWeight: FontWeight.w500,
                            ...
                            onCancel: () async {
                              final confirmed = await showDialog<bool>(
                                ...
                                builder: (dialogContext) => AlertDialog(
                                  ...
                                  content: Text(
                                    t.dcCancelDonationBody(d.consumerName),
                                  ),
                                  actions: [
                                    TextButton(
                                      onPressed: () =>
                                          Navigator.pop(dialogContext, false),
                                      child: Text(t.dcKeepIt),
                                    ),
                                    TextButton(
                                      onPressed: () =>
                                          Navigator.pop(dialogContext, true),
                                      child: Text(t.dcCancelDonation),
                                    ),
                                  ],
                                ),
                              );
                        ...
                        onConfirm:
                            (
                              ...
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text(
                                    t.dcDonationScheduledFor(account.name),
                                  ),
                                  backgroundColor: const Color(0xFF16A34A),
                                ),
                              );
...
class _UpcomingDonationRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final formatted =
        '${t.hour.toString().padLeft(2, '0')}:${t.minute.toString().padLeft(2, '0')} · ${t.day}/${t.month}/${t.year}';
    return Container(
      ...
      child: Row(
        children: [
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        context.l10n.dcQtyItemCategory(
                          donation.itemName,
                          donation.category,
                          donation.quantity,
                        ),
          ...
          IconButton(
            ...
            onPressed: onEdit,
            tooltip: context.l10n.dcEdit,
          ),
          IconButton(
            icon: const Icon(Icons.close, size: 18),
            onPressed: onCancel,
            tooltip: context.l10n.dcCancel,
          ),
        ],
      ),
    );
    ...
    return _HoverScale(
      child: GestureDetector(
        onTap: () => showListingDetailSheet(
          context,
          title: account.name,
          subtitle: isAvailable ? t.dcAvailable : t.dcUnavailable,
          donorId: account.uid,
          rows: [
            DetailRow(
              Icons.star_outline,
              'Rating',
              '${account.rating} (${account.reviewCount} reviews)',
            ),
            DetailRow(
              pickupPreferenceIcon(account.pickupPreference),
              'Pickup',
              pickupPreferenceLabel(t, account.pickupPreference),
            ),
            DetailRow(
              Icons.location_on_outlined,
              'Location',
            ...
            DetailRow(
              ...
              'Donated',
              streak > 0 ? t.dcTimesDonated(streak) : t.dcNew,
            ),
          ],
          menuActions: [
            blockSheetMenuItem(
